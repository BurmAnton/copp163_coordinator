...
function set_average_cost(table){
    ...
    table.querySelectorAll('.program_sum').forEach(input =>{
        ...
        sum_td.innerHTML = `${price.value*quota.value} руб`;
        programs_sum += price.value*quota.value;
        if (quota.value != 0) {
    ...
    if (table.querySelector('.quota_count') != null) {
        ...
        table.querySelector('.bracket_sum').innerHTML = `${programs_sum} руб`;
...
function set_average_quota_cost(){
    ...
    document.querySelector('.bracket_sum_quota').innerHTML = `${programs_sum} руб`;
