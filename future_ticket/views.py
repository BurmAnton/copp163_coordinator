...
def quotas(request):
    ...
    for ter_admin in School.TER_CHOICES:
        ...
        ter_admin_schools = School.objects.filter(
            territorial_administration=ter_admin[0]
        )
        schools_quota = TicketQuota.objects.filter(
            school__in=ter_admin_schools).distinct()
        quota_stat[ter_admin[1]]['federal_quota'] = schools_quota.filter(
            is_federal=True).distinct().aggregate(Sum("approved_value"))['approved_value__sum']
        if quota_stat[ter_admin[1]]['federal_quota'] == None:
            quota_stat[ter_admin[1]]['federal_quota'] = 0
        quota_stat[ter_admin[1]]['none_federal_quota'] = schools_quota.filter(
            is_federal=False).distinct().aggregate(Sum("approved_value"))['approved_value__sum']
        if quota_stat[ter_admin[1]]['none_federal_quota'] == None:
            quota_stat[ter_admin[1]]['none_federal_quota'] = 0
        quota_stat[ter_admin[1]]['full_quota'] = schools_quota.distinct().aggregate(
            Sum("approved_value"))['approved_value__sum']
        if quota_stat[ter_admin[1]]['full_quota'] == None:
            quota_stat[ter_admin[1]]['full_quota'] = 0
        quota_stat_all['full_qouta'] += quota_stat[ter_admin[1]]['full_quota']
        quota_stat_all['federal_quota'] += quota_stat[ter_admin[1]]['federal_quota']
        quota_stat_all['none_federal_quota'] += quota_stat[ter_admin[1]]['none_federal_quota']
